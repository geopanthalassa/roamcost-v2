...
const TRANSPORT_OPTIONS = [
    { id: 'public', label: 'Public Transport', multiplier: 1.0, desc: 'Bus, metro, train' },
    { id: 'uber', label: 'Uber / Taxi', multiplier: 3.5, desc: 'Daily rides' },
    { id: 'rental', label: 'Rent a Car', multiplier: 5.0, desc: 'Monthly rental + fuel' },
    { id: 'own', label: 'Own Vehicle', multiplier: 4.0, desc: 'Fuel + maintenance' },
    { id: 'bike', label: 'Bike / Walk', multiplier: 0.2, desc: 'Minimal cost' },
];

const FOOD_OPTIONS = [
    { id: 'market', label: 'Supermarket', multiplier: 0.7, desc: 'Cook at home mostly' },
    { id: 'mixed', label: 'Mixed', multiplier: 1.0, desc: 'Half home, half out' },
    { id: 'restaurants', label: 'Restaurants', multiplier: 2.2, desc: 'Eat out daily' },
    { id: 'delivery', label: 'Food Delivery', multiplier: 2.8, desc: 'Apps like Uber Eats' },
];

const HOUSING_OPTIONS = [
    { id: 'hostel', label: 'Hostel / Shared', multiplier: 0.4, desc: 'Budget option' },
    { id: 'airbnb', label: 'Airbnb', multiplier: 1.6, desc: 'Short-term stays' },
    { id: 'rental', label: 'Long-term Rental', multiplier: 1.0, desc: 'Standard apartment' },
    { id: 'luxury', label: 'Luxury / Serviced', multiplier: 2.5, desc: 'High-end apartment' },
];

const LIFESTYLE_OPTIONS = [
    { id: 'budget', label: 'Budget', multiplier: 0.7, desc: 'Minimal spending' },
    { id: 'midrange', label: 'Mid-range', multiplier: 1.0, desc: 'Comfortable living' },
    { id: 'luxury', label: 'Luxury', multiplier: 2.0, desc: 'Premium everything' },
];
...
function OptionButton({ option, selected, onSelect, color }: {
    ...
    return (
        <button onClick={onSelect} style={{
            ...
            <span style={{ fontSize: '0.7rem', fontWeight: 800, color: selected ? color : '#475569', textAlign: 'center', lineHeight: 1.2 }}>{option.label}</span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div style={{ textAlign: 'left' }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: '#64748b' }}>
                            {open ? 'Adjust your lifestyle' : `~$${total.toLocaleString()}/mo based on your style`}
                        </div>
                    </div>
                </div>
                ...
                <div style={{ padding: '1.5rem' }}>
                    <div style={{ marginBottom: '1.25rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                            {TRANSPORT_OPTIONS.map(o => <OptionButton key={o.id} option={o} selected={transport === o.id} onSelect={() => setTransport(o.id)} color={GREEN} />)}
                        </div>
                    </div>
                    <div style={{ marginBottom: '1.25rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                            {FOOD_OPTIONS.map(o => <OptionButton key={o.id} option={o} selected={food === o.id} onSelect={() => setFood(o.id)} color={ORANGE} />)}
                        </div>
                    </div>
                    <div style={{ marginBottom: '1.25rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                            {HOUSING_OPTIONS.map(o => <OptionButton key={o.id} option={o} selected={housing === o.id} onSelect={() => setHousing(o.id)} color='#3b82f6' />)}
                        </div>
                    </div>
                    <div style={{ marginBottom: '1.5rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.5rem' }}>
                            {LIFESTYLE_OPTIONS.map(o => <OptionButton key={o.id} option={o} selected={lifestyle === o.id} onSelect={() => setLifestyle(o.id)} color='#8b5cf6' />)}
                        </div>
                    </div>
                        ...
                        {[
                            ...
                            <div key={item.label} style={{ marginBottom: '0.625rem' }}>
                                ...
                                <div style={{ height: '5px', backgroundColor: '#e2e8f0', borderRadius: '3px', overflow: 'hidden' }}>
                                    <div style={{ height: '100%', width: `${Math.min((item.value / total) * 100, 100)}%`, backgroundColor: item.color, borderRadius: '3px', transition: 'width 0.3s' }} />
                                </div>
                            </div>
                        ))}
