...
function getWeatherDesc(code: number): { label: string; color: string } {
    if (code === 0) return { label: 'Clear sky', color: '#e8833a' };
    ...
    if (code <= 99) return { label: 'Thunderstorm', color: '#e8833a' };
    return { label: 'Unknown', color: '#94a3b8' };
}

const DAYS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

const SunIcon = ({ color = '#e8833a' }: { color?: string }) => (
...
function WeatherIcon({ code, size = 18 }: { code: number; size?: number }) {
    if (code === 0) return <SunIcon color="#e8833a" />;
                ...
                {daily.time.slice(0, 7).map((date, i) => {
                    ...
                    return (
                        <div key={date} style={{
                            ...
                            <div style={{ fontSize: '0.58rem', fontWeight: 700, color: i === 0 ? '#5b8c71' : '#94a3b8', textTransform: 'uppercase', marginBottom: '0.25rem' }}>{dayName}</div>
