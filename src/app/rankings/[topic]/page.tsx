...
export default async function RankingPage({ params }: RankingPageProps) {
    ...
    let query = supabase.from('cities_master').select('*').gt('cost_index', 0).not('population', 'is', null).limit(300);
    let title = 'City Rankings';
    let description = 'Discover top destinations based on your preferences.';

    if (topic === 'cheapest') {
        query = query.gt('population', 1000000).gt('rent_index', 0).gt('safety', 0).order('rent_index', { ascending: true });
        title = 'Value Leaders';
        description = 'Global hubs where your budget stretches the furthest.';
    } else if (topic === 'nomads') {
        query = query.gt('population', 1000000).gt('internet', 0).gt('safety', 0).order('internet', { ascending: false });
        title = 'Connectivity Hubs';
        description = 'Leading cities for digital work and high-speed infrastructure.';
    } else if (topic === 'safest') {
        query = query.gt('population', 1000000).gt('safety', 0).gt('rent_index', 0).order('safety', { ascending: false });
        title = 'Safest Global Cities';
        description = 'Secured metropolitan areas with the highest safety ratings.';
    } else if (topic === 'quality') {
        query = query.gt('population', 1000000).gt('cost_index', 0).gt('safety', 0).gt('rent_index', 0).order('cost_index', { ascending: false });
