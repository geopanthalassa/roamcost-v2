...
function CityCard({ city, total, isWinner }: { city: City; total: number; isWinner: boolean }) {
    const h = Math.abs(city.slug.split('').reduce((a, c) => ((a << 5) - a) + c.charCodeAt(0), 0));
    const fallback = `https://images.unsplash.com/${POOL[h % POOL.length]}?auto=format&fit=crop&w=800&h=500&q=80`;

    return (
        <Link href={`/city/${city.slug}`} style={{ textDecoration: 'none' }}>
            <div style={{ backgroundColor: 'white', borderRadius: '1rem', overflow: 'hidden', border: isWinner ? `2px solid ${GREEN}` : '1px solid #e2e8f0', transition: 'transform 0.15s, box-shadow 0.15s' }}>
                {/* City photo */}
                <div style={{ height: '180px', overflow: 'hidden', position: 'relative' }}>
                    ...
                    {isWinner && (
                        <div style={{ position: 'absolute', top: '10px', right: '10px', backgroundColor: GREEN, color: 'white', fontSize: '0.6rem', fontWeight: 900, padding: '0.2rem 0.6rem', borderRadius: '2rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                            ✓ Cheapest
                        </div>
                    )}
                    <div style={{ position: 'absolute', bottom: '12px', left: '12px' }}>
                        <div style={{ fontSize: '0.62rem', color: 'rgba(255,255,255,0.6)', fontWeight: 600, marginBottom: '0.1rem' }}>{city.country}</div>
                        <div style={{ fontSize: '1.1rem', fontWeight: 900, color: 'white' }}>{city.city}</div>
                    </div>
                </div>
                {/* Cost */}
                <div style={{ padding: '1rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                        <div style={{ fontSize: '0.68rem', color: '#94a3b8', fontWeight: 600, marginBottom: '0.15rem' }}>Est. monthly cost</div>
                        <div style={{ fontSize: '1.3rem', fontWeight: 900, color: isWinner ? GREEN : '#0f172a' }}>
                            {total > 0 ? `$${total.toLocaleString()}` : '—'}
                        </div>
                    </div>
                    <div style={{ fontSize: '0.75rem', fontWeight: 700, color: GREEN }}>
                        View full profile →
                    </div>
                </div>
            </div>
    ...
    useEffect(() => {
        async function load() {
            ...
            try {
                ...
                const ordered = citySlugList.map(s => arr.find((c: City) => c.slug === s)).filter(Boolean) as City[];
        ...
        <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ backgroundColor: 'white', borderRadius: '1.25rem', padding: '3rem', textAlign: 'center', border: '1px solid #e2e8f0', maxWidth: '480px' }}>
                ...
                <p style={{ color: '#64748b', marginBottom: '2rem', lineHeight: 1.6, fontSize: '0.88rem' }}>
                    We couldn't find <strong>{notFound.join(', ')}</strong> in our database.
                </p>
        ...
        <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc' }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', padding: '3rem 0 2.5rem' }}>
                <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 1.5rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', marginBottom: '1rem', fontSize: '0.75rem' }}>
                    ...
                    {/* City names prominent at top */}
                    <h1 style={{ fontSize: '2.25rem', fontWeight: 900, color: 'white', margin: '0 0 0.4rem', letterSpacing: '-0.04em', lineHeight: 1.1 }}>
                        {cities.map((c, i) => (
                            <span key={c.slug}>
                                {i > 0 && <span style={{ color: 'rgba(255,255,255,0.3)', fontSize: '1.5rem', margin: '0 0.5rem' }}>vs</span>}
                                {c.city}
                            </span>
                        ))}
                    </h1>
            ...
            <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '2rem 1.5rem' }}>
                {/* Not found warning */}
                {notFound.length > 0 && (
                    <div style={{ backgroundColor: '#fef3c7', border: '1px solid #fde68a', borderRadius: '0.75rem', padding: '0.75rem 1.25rem', marginBottom: '1.5rem', fontSize: '0.8rem', color: '#92400e' }}>
                        ⚠️ Not found: <strong>{notFound.join(', ')}</strong> — showing available cities.
                    </div>
                )}
                ...
                <div style={{ backgroundColor: 'white', borderRadius: '1.25rem', border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '2rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: `185px repeat(${cols}, 1fr)`, backgroundColor: '#0f172a' }}>
                        <div style={{ padding: '1rem 1.25rem', display: 'flex', alignItems: 'center' }}>
                            <span style={{ fontSize: '0.8rem', fontWeight: 800, color: 'rgba(255,255,255,0.65)' }}>Est. Monthly Total</span>
                        </div>
                        {cities.map((city, i) => (
