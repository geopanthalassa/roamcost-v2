...
export default async function ComparePage({ params }: Props) {
    const { slugs } = await params;
    const citySlugList = slugs.split('-vs-').filter(Boolean).slice(0, 4);
    if (citySlugList.length < 2) notFound();
    ...
    const ordered = citySlugList.map(s => cities.find(c => c.slug === s)).filter(Boolean) as City[];
            ...
            <div style={{ minHeight: '100vh', backgroundColor: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ backgroundColor: 'white', borderRadius: '1.25rem', padding: '4rem', textAlign: 'center', border: '1px solid #e2e8f0', maxWidth: '500px' }}>
                    ...
                    <p style={{ color: '#64748b', marginBottom: '2rem', lineHeight: 1.6 }}>
                        ...
                        Try searching for the cities individually first.
                    </p>
                    <Link href="/compare" style={{ backgroundColor: GREEN, color: 'white', padding: '0.75rem 1.5rem', borderRadius: '0.75rem', fontWeight: 700, textDecoration: 'none', display: 'inline-block' }}>
                        Try another comparison →
                    </Link>
    ...
    const images = await Promise.all(
        ordered.map(c => getCityImageServer(c.slug, c.city, c.country || '', 800))
    );

    const METRICS = [
        { label: 'Monthly Rent', key: 'rent_index', better: 'lower', format: 'money', desc: '1BR apartment' },
        { label: 'Food / month', key: 'food_index', better: 'lower', format: 'money', factor: 30, desc: 'Groceries + eating out' },
        { label: 'Transport', key: 'transport_index', better: 'lower', format: 'money', desc: 'Monthly pass' },
        { label: 'Utilities', key: 'utilities_index', better: 'lower', format: 'money', desc: 'Electricity, water, internet' },
        { label: 'Safety', key: 'safety', better: 'higher', format: 'score', desc: 'Out of 10' },
        { label: 'Internet', key: 'internet', better: 'higher', format: 'mbps', desc: 'Average Mbps' },
        { label: 'Healthcare', key: 'healthcare', better: 'higher', format: 'score', desc: 'Out of 10' },
        { label: 'Quality of Life', key: 'cost_index', better: 'higher', format: 'score', desc: 'Overall score' },
    ];
    ...
    const getBest = (key: string, better: string, factor = 1) => {
        const vals = ordered.map(c => getVal(c, key, factor));
        const valid = vals.filter(v => v != null) as number[];
        if (!valid.length) return null;
        return better === 'lower' ? Math.min(...valid) : Math.max(...valid);
    };
    ...
    const totalCost = (city: City) => {
        const rent = getVal(city, 'rent_index') || 0;
        const food = getVal(city, 'food_index', 30) || 0;
        const transport = getVal(city, 'transport_index') || 0;
        const utils = getVal(city, 'utilities_index') || 0;
        return Math.round(rent + food + transport + utils);
    };

    const totals = ordered.map(totalCost);
    const minTotal = Math.min(...totals.filter(t => t > 0));
            ...
            <div style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', padding: '3rem 0 0' }}>
                <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 1.5rem' }}>
                    <nav style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', marginBottom: '1.5rem' }}>
                        ...
                        <Link href="/compare" style={{ color: 'rgba(255,255,255,0.4)', textDecoration: 'none' }}>Compare</Link>
                        {' → '}
                        <span style={{ color: 'rgba(255,255,255,0.7)' }}>{ordered.map(c => c.city).join(' vs ')}</span>
                    </nav>

                    <h1 style={{ fontSize: '2rem', fontWeight: 900, color: 'white', margin: '0 0 0.5rem', letterSpacing: '-0.03em' }}>
                        {ordered.map(c => c.city).join(' vs ')}
                    </h1>
                    ...
                    {/* City header cards */}
                    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${ordered.length}, 1fr)`, gap: '1px', backgroundColor: '#334155' }}>
                        {ordered.map((city, i) => (
                            <div key={city.slug} style={{ backgroundColor: '#0f172a', padding: '1.25rem', position: 'relative', overflow: 'hidden' }}>
                                <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${images[i]})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.2 }} />
                                <div style={{ position: 'relative', zIndex: 1 }}>
                                    <div style={{ fontSize: '0.65rem', fontWeight: 700, color: 'rgba(255,255,255,0.4)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.25rem' }}>{city.country}</div>
                                    <div style={{ fontSize: '1.2rem', fontWeight: 900, color: 'white', marginBottom: '0.5rem' }}>{city.city}</div>
                                    <div style={{ fontSize: '1.5rem', fontWeight: 900, color: totals[i] === minTotal && totals[i] > 0 ? GREEN : 'white' }}>
                                        {totals[i] > 0 ? `$${totals[i].toLocaleString()}` : '—'}
                                        <span style={{ fontSize: '0.75rem', fontWeight: 600, color: 'rgba(255,255,255,0.5)', marginLeft: '0.25rem' }}>/mo</span>
                                    </div>
                                    {totals[i] === minTotal && totals[i] > 0 && (
                                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', backgroundColor: '#52B78820', border: '1px solid #52B78840', borderRadius: '2rem', padding: '0.15rem 0.5rem', marginTop: '0.4rem' }}>
                                            <span style={{ fontSize: '0.6rem', fontWeight: 800, color: GREEN, textTransform: 'uppercase' }}>Cheapest</span>
                                        </div>
                                    )}
                                </div>
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '2rem 1.5rem' }}>
                ...
                {ordered.length < 4 && (
                    <div style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'flex-end' }}>
                        <Link href="/compare" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '0.625rem', padding: '0.5rem 1rem', fontSize: '0.8rem', fontWeight: 700, color: '#475569', textDecoration: 'none' }}>
                            <span style={{ width: '18px', height: '18px', borderRadius: '50%', backgroundColor: GREEN, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.9rem', fontWeight: 900 }}>+</span>
                            Add another city
                        </Link>
                    </div>
                )}
                ...
                <div style={{ backgroundColor: 'white', borderRadius: '1.25rem', border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '2rem' }}>
                    {/* Table header */}
                    <div style={{ display: 'grid', gridTemplateColumns: `200px repeat(${ordered.length}, 1fr)`, backgroundColor: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                        <div style={{ padding: '1rem 1.25rem', fontSize: '0.72rem', fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Category</div>
                        {ordered.map(city => (
                            <div key={city.slug} style={{ padding: '1rem', textAlign: 'center', borderLeft: '1px solid #e2e8f0' }}>
                                <Link href={`/city/${city.slug}`} style={{ fontSize: '0.9rem', fontWeight: 900, color: '#0f172a', textDecoration: 'none' }}>{city.city}</Link>
                                <div style={{ fontSize: '0.65rem', color: '#94a3b8', marginTop: '0.1rem' }}>{city.country}</div>
                    ...
                    {METRICS.map((metric, mi) => {
                        const factor = (metric as any).factor || 1;
                        const best = getBest(metric.key, metric.better, factor);
                        const vals = ordered.map(c => getVal(c, metric.key, factor));

                        return (
                            <div key={metric.key} style={{ display: 'grid', gridTemplateColumns: `200px repeat(${ordered.length}, 1fr)`, borderBottom: mi < METRICS.length - 1 ? '1px solid #f1f5f9' : 'none', backgroundColor: mi % 2 === 0 ? 'white' : '#fafbfc' }}>
                                <div style={{ padding: '1rem 1.25rem', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                    <div style={{ fontSize: '0.85rem', fontWeight: 700, color: '#0f172a' }}>{metric.label}</div>
                                    <div style={{ fontSize: '0.68rem', color: '#94a3b8', marginTop: '0.1rem' }}>{metric.desc}</div>
                                </div>
                                {vals.map((val, i) => {
                                    const isBest = val != null && val === best;
                                    return (
                                        <div key={i} style={{ padding: '1rem', textAlign: 'center', borderLeft: '1px solid #e2e8f0', backgroundColor: isBest ? '#F0FAF4' : 'transparent', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.3rem' }}>
                                            <span style={{ fontSize: '1rem', fontWeight: isBest ? 900 : 600, color: isBest ? GREEN : '#0f172a' }}>
                                                {formatValue(val, metric.format)}
                                            </span>
                                            {isBest && val != null && (
                                                <span style={{ fontSize: '0.55rem', backgroundColor: GREEN, color: 'white', padding: '0.1rem 0.4rem', borderRadius: '2rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                                                    {metric.better === 'lower' ? 'Cheapest' : 'Best'}
                                                </span>
                                            )}
                                        </div>
                    ...
                    {/* Total row */}
                    <div style={{ display: 'grid', gridTemplateColumns: `200px repeat(${ordered.length}, 1fr)`, backgroundColor: '#0f172a' }}>
                        <div style={{ padding: '1.25rem', display: 'flex', alignItems: 'center' }}>
                            <span style={{ fontSize: '0.85rem', fontWeight: 800, color: 'white' }}>Est. Monthly Total</span>
                        </div>
                        {ordered.map((city, i) => (
                            <div key={city.slug} style={{ padding: '1.25rem', textAlign: 'center', borderLeft: '1px solid #1e293b' }}>
                                <div style={{ fontSize: '1.25rem', fontWeight: 900, color: totals[i] === minTotal && totals[i] > 0 ? GREEN : 'white' }}>
                                ...
                                {totals[i] > 0 && totals[i] !== minTotal && minTotal > 0 && (
                                    <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.4)', marginTop: '0.2rem' }}>
                                        +${(totals[i] - minTotal).toLocaleString()}/mo more
                                    </div>
                                )}
                            </div>
                        ))}
                ...
                {/* City links */}
                <div style={{ display: 'grid', gridTemplateColumns: `repeat(${ordered.length}, 1fr)`, gap: '1rem', marginBottom: '2rem' }}>
                    {ordered.map(city => (
                        <Link key={city.slug} href={`/city/${city.slug}`}
                            style={{ backgroundColor: 'white', border: '1px solid #e2e8f0', borderRadius: '0.875rem', padding: '1.25rem', textDecoration: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                                <div style={{ fontSize: '0.9rem', fontWeight: 800, color: '#0f172a' }}>{city.city}</div>
                                <div style={{ fontSize: '0.75rem', color: '#94a3b8', marginTop: '0.15rem' }}>See full city profile</div>
                            </div>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={GREEN} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                        </Link>
                    ))}
                ...
                <div style={{ backgroundColor: '#0f172a', borderRadius: '1.25rem', padding: '2.5rem', textAlign: 'center' }}>
                    <h2 style={{ color: 'white', fontSize: '1.5rem', fontWeight: 900, margin: '0 0 0.5rem', letterSpacing: '-0.03em' }}>Compare other cities</h2>
                    <p style={{ color: 'rgba(255,255,255,0.45)', margin: '0 0 1.5rem', fontSize: '0.875rem' }}>Add up to 4 cities side by side</p>
                    <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
